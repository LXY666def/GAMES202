...
VkResult VulkanExampleBase::createInstance()
{
	...
	// set up debug messenger and extension
...
bool VulkanExampleBase::initVulkan()
{
	...
	std::cout << "Found GPU count: " << gpuCount << std::endl;
	for (uint32_t i = 0; i < gpuCount; i++) {
		VkPhysicalDeviceProperties deviceProperties;
		vkGetPhysicalDeviceProperties(physicalDevices[i], &deviceProperties);
		if (deviceProperties.vendorID == 4318u) {
			selectedDevice = i;
		}	// nvidia gpu
	...
	std::cout << "Choosing Device [" << selectedDevice << "]" << std::endl;

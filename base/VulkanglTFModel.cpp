...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags,
	float scale, std::vector<Vertex>* pVertexHost, std::vector<uint32_t>* pIndexHost)
{
	...
	std::vector<uint32_t> indexBufferTmp;
	std::vector<Vertex> vertexBufferTmp;
	std::vector<uint32_t>& indexBuffer = pIndexHost ? *pIndexHost : indexBufferTmp;
	std::vector<Vertex>& vertexBuffer = pVertexHost ? *pVertexHost : vertexBufferTmp;

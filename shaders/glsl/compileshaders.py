...
parser.add_argument('--p', type=str, help='only compile one project')
...
file_extensions = tuple([".vert", ".frag", ".comp", ".geom", ".tesc", ".tese", ".rgen", ".rahit", ".rchit", ".rmiss", ".mesh", ".task"])

glslang_path = findGlslang()
dir_path = os.path.dirname(os.path.realpath(__file__))
dir_path = dir_path.replace('\\', '/')
for root, dirs, files in os.walk(dir_path):
    dir_tmp = root.replace('\\', ' ').split()[-1]
    if args.p is not None and dir_tmp != args.p:
        continue
    ...
    for file in files:
        if file.endswith(file_extensions):
            ...
            if file.endswith(".rgen") or file.endswith(".rchit") or file.endswith(".rmiss"):
               add_params = add_params + " --target-env vulkan1.3"
            if file.endswith(".rahit"):
               add_params = add_params + " --target-env vulkan1.3"
